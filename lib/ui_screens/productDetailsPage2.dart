...
class ProductDetail2 extends StatefulWidget {
  final slug;
  ProductDetail2(this.slug);
  @override
  State<StatefulWidget> createState() {
    return _ProductDetail2();
  }
}
...
class _ProductDetail2 extends State<ProductDetail2> {
  ...
  void initState() {
    ...
    text = _details['dynamic-link'];
    model = _details["model"];
        ...
        child: Column(
          children: <Widget>[
            ...
            Container(
              ...
              width: double.infinity,
              child: Text(
                _details['price'].toString() + " INR",
                style: ThemeApp().textSemiBoldTheme(),
                textAlign: TextAlign.left,
              ),
            ),
            _details.containsKey("km")
                ? Container(
                    padding: EdgeInsets.fromLTRB(17, 5, 10, 5),
                    width: double.infinity,
                    child: Text(
                        "Kilometers : " + _details['km'].toString() + " km",
                        style: ThemeApp().textSemiBoldTheme(),
                        textAlign: TextAlign.left),
                  )
                : Container(),
            Container(
              padding: EdgeInsets.fromLTRB(17, 5, 10, 5),
              width: double.infinity,
              child: Text("Color : " + _details['color'],
                  style: ThemeApp().textSemiBoldTheme(),
                  textAlign: TextAlign.left),
            ),
            Container(
              padding: EdgeInsets.fromLTRB(17, 5, 10, 5),
              width: double.infinity,
              child: Text("Year : " + _details['year'],
                  style: ThemeApp().textSemiBoldTheme(),
                  textAlign: TextAlign.left),
            ),
            _details.containsKey("fuel_type")
                ? Container(
                    padding: EdgeInsets.fromLTRB(17, 5, 10, 5),
                    width: double.infinity,
                    child: Text("Fuel : " + _details['fuel_type'],
                        style: ThemeApp().textSemiBoldTheme(),
                        textAlign: TextAlign.left),
                  )
                : Container(),
            _details.containsKey("type")
                ? Container(
                    padding: EdgeInsets.fromLTRB(17, 5, 10, 5),
                    width: double.infinity,
                    child: Text("Type : " + _details['type'],
                        style: ThemeApp().textSemiBoldTheme(),
                        textAlign: TextAlign.left),
                  )
                : Container(),
            ...
            RaisedButton(
              onPressed: () async {
                ...
                var X = geo.latitude;
                var Y = geo.longitude;
                mp = 'http://maps.google.com?q=$X,$Y';

                if (!await canLaunch(mp)) {
                  throw 'Could not launch $mp';
                } else {
                  await launch(mp);
                }
              },
              child: Text("Visit From Map"),
            )
          ],
        ),
      ),
      ...
      child: Container(
        decoration: BoxDecoration(
            image: DecorationImage(
                fit: BoxFit.fitWidth,
                image: NetworkImage(hostUrl + imgList[index]))),
      ),
    ));
  }
                  ...
                  new Container(
                      ...
                      decoration: new BoxDecoration(
                          ...
                          image: new DecorationImage(
                              fit: BoxFit.fill,
                              image: dealerData['image'] == null
                                  ? AssetImage('assets/user.png')
                                  : new NetworkImage(dealerData['image'])))),
                  SizedBox(
                    width: 30,
                  ),
                  Container(
                    ...
                    onTap: () {
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (context) => DealerProfilePage(dealerData)));
                    },
                    child: Text(
                      dealerData['shop_name'],
  ...
  fetchProductDetails() async {
    Dio dio = Dio();
    var response = await dio.get(itemDetails,
        queryParameters: {"slug": _slug},
        options: Options(
          contentType: 'application/json',
        ));
